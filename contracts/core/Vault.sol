...
contract Vault is ReentrancyGuard, IVault {
    ...
    address public override wrapper;
    ...
    // usdrAmounts tracks the amount of USDR debt for each whitelisted token
    mapping (address => uint256) public override usdrAmounts;

    // maxUsdrAmounts allows setting a max amount of USDR debt for a token
    mapping (address => uint256) public override maxUsdrAmounts;
    ...
    // this value is used to calculate the redemption values for selling of USDR
    ...
    event BuyUSDR(address account, address token, uint256 tokenAmount, uint256 usdrAmount, uint256 feeBasisPoints);
    event SellUSDR(address account, address token, uint256 usdrAmount, uint256 tokenAmount, uint256 feeBasisPoints);
    ...
    event IncreaseUsdrAmount(address token, uint256 amount);
    event DecreaseUsdrAmount(address token, uint256 amount);
    ...
    // setWrapper sets the wrapper contract
    ...
    function setMaxLeverage(uint256 _maxLeverage) external override {
        _onlyGov();
        require(_maxLeverage >= MIN_LEVERAGE, "2");
    ...
    function setFees(
        ...
        require(_stableTaxBasisPoints <= MAX_FEE_BASIS_POINTS, "4");
        require(_mintBurnFeeBasisPoints <= MAX_FEE_BASIS_POINTS, "5");
        require(_swapFeeBasisPoints <= MAX_FEE_BASIS_POINTS, "6");
        require(_stableSwapFeeBasisPoints <= MAX_FEE_BASIS_POINTS, "7");
        require(_marginFeeBasisPoints <= MAX_FEE_BASIS_POINTS, "8");
        require(_liquidationFeeUsd <= MAX_LIQUIDATION_FEE_USD, "9");
    ...
    function setFundingRate(uint256 _fundingInterval, uint256 _fundingRateFactor, uint256 _stableFundingRateFactor) external override {
        _onlyGov();
        require(_fundingInterval >= MIN_FUNDING_RATE_INTERVAL, "10");
        require(_fundingRateFactor <= MAX_FUNDING_RATE_FACTOR, "11");
        require(_stableFundingRateFactor <= MAX_FUNDING_RATE_FACTOR, "12");
    ...
    function setTokenConfig(
        ...
        maxUsdrAmounts[_token] = _maxUsdrAmount;
    ...
    function clearTokenConfig(address _token) external {
        _onlyGov();
        require(whitelistedTokens[_token], "13");
    ...
    function setUsdrAmount(address _token, uint256 _amount) external override {
        _onlyGov();

        uint256 usdrAmount = usdrAmounts[_token];
        if (_amount > usdrAmount) {
            _increaseUsdrAmount(_token, _amount.sub(usdrAmount));
            return;
        }

        _decreaseUsdrAmount(_token, usdrAmount.sub(_amount));
    }
    ...
    // deposit into the pool without minting USDR tokens
    // useful in allowing the pool to become over-collaterised
    function directPoolDeposit(address _token) external override nonReentrant {
        require(whitelistedTokens[_token], "14");
    ...
    function buyUSDR(address _token, address _receiver) external override nonReentrant returns (uint256) {
        _validateManager();
        ...
        uint256 feeBasisPoints = getFeeBasisPoints(_token, usdrAmount, mintBurnFeeBasisPoints, taxBasisPoints, true);
        ...
        emit BuyUSDR(_receiver, _token, tokenAmount, mintAmount, feeBasisPoints);

        useSwapPricing = false;
        return mintAmount;
    }

    function sellUSDR(address _token, address _receiver) external override nonReentrant returns (uint256) {
        _validateManager();
        ...
        // the _transferIn call increased the value of tokenBalances[usdr]
        // usually decreases in token balances are synced by calling _transferOut
        // however, for usdr, the tokens are burnt, so _updateTokenBalance should
        // be manually called to record the decrease in tokens
        _updateTokenBalance(usdr);

        uint256 feeBasisPoints = getFeeBasisPoints(_token, usdrAmount, mintBurnFeeBasisPoints, taxBasisPoints, false);
        uint256 amountOut = _collectSwapFees(_token, redemptionAmount, feeBasisPoints);
        require(amountOut > 0, "22");

        _transferOut(_token, amountOut, _receiver);

        emit SellUSDR(_receiver, _token, usdrAmount, amountOut, feeBasisPoints);
    ...
    function swap(address _tokenIn, address _tokenOut, address _receiver) external override nonReentrant returns (uint256) {
        ...
        // adjust usdrAmounts by the same usdrAmount as debt is shifted between the assets
        uint256 usdrAmount = amountIn.mul(priceIn).div(PRICE_PRECISION);
        usdrAmount = adjustForDecimals(usdrAmount, _tokenIn, usdr);
        ...
        uint256 feeBasisPoints;
        {
            ...
            uint256 feesBasisPoints0 = getFeeBasisPoints(_tokenIn, usdrAmount, baseBps, taxBps, true);
            uint256 feesBasisPoints1 = getFeeBasisPoints(_tokenOut, usdrAmount, baseBps, taxBps, false);
        ...
        _increaseUsdrAmount(_tokenIn, usdrAmount);
        _decreaseUsdrAmount(_tokenOut, usdrAmount);
    ...
    function increasePosition(address _account, address _collateralToken, address _indexToken, uint256 _sizeDelta, bool _isLong) external override nonReentrant {
        ...
        require(position.collateral >= fee, "29");
    ...
    function _decreasePosition(address _account, address _collateralToken, address _indexToken, uint256 _collateralDelta, uint256 _sizeDelta, bool _isLong, address _receiver) private returns (uint256) {
        ...
        bytes32 key = getPositionKey(_account, _collateralToken, _indexToken, _isLong);
        Position storage position = positions[key];
        require(position.size > 0, "31");
        require(position.size >= _sizeDelta, "32");
        require(position.collateral >= _collateralDelta, "33");
    ...
    function liquidatePosition(address _account, address _collateralToken, address _indexToken, bool _isLong, address _feeReceiver) external override nonReentrant {
        if (inPrivateLiquidationMode) {
            require(isLiquidator[msg.sender], "34");
        }
    ...
    function validateLiquidation(address _account, address _collateralToken, address _indexToken, bool _isLong, bool _raise) public override view returns (uint256, uint256) {
        bytes32 key = getPositionKey(_account, _collateralToken, _indexToken, _isLong);
        Position memory position = positions[key];
    ...
    function getRedemptionAmount(address _token, uint256 _usdrAmount) public override view returns (uint256) {
        uint256 price = getMaxPrice(_token);
        uint256 redemptionAmount = _usdrAmount.mul(PRICE_PRECISION).div(price);
        return adjustForDecimals(redemptionAmount, usdr, _token);
    }
    ...
    function adjustForDecimals(uint256 _amount, address _tokenDiv, address _tokenMul) public view returns (uint256) {
        uint256 decimalsDiv = _tokenDiv == usdr ? USDR_DECIMALS : tokenDecimals[_tokenDiv];
        uint256 decimalsMul = _tokenMul == usdr ? USDR_DECIMALS : tokenDecimals[_tokenMul];
    ...
    function getPositionLeverage(address _account, address _collateralToken, address _indexToken, bool _isLong) public view returns (uint256) {
        bytes32 key = getPositionKey(_account, _collateralToken, _indexToken, _isLong);
        Position memory position = positions[key];
        require(position.collateral > 0, "37");
    ...
    function getFeeBasisPoints(address _token, uint256 _usdrDelta, uint256 _feeBasisPoints, uint256 _taxBasisPoints, bool _increment) public override view returns (uint256) {
        if (!hasDynamicFees) { return _feeBasisPoints; }

        uint256 initialAmount = usdrAmounts[_token];
        uint256 nextAmount = initialAmount.add(_usdrDelta);
        if (!_increment) {
            nextAmount = _usdrDelta > initialAmount ? 0 : initialAmount.sub(_usdrDelta);
        }

        uint256 targetAmount = getTargetUsdrAmount(_token);
    ...
    function getTargetUsdrAmount(address _token) public override view returns (uint256) {
        uint256 supply = IERC20(usdr).totalSupply();
    ...
    function _validatePosition(uint256 _size, uint256 _collateral) private pure {
        if (_size == 0) {
            require(_collateral == 0, "39");
            return;
        }
        require(_size >= _collateral, "40");
    }

    function _validateRouter(address _account) private view {
        if (msg.sender == _account) { return; }
        if (msg.sender == router) { return; }
        require(approvedRouters[_account][msg.sender], "41");
    }

    function _validateTokens(address _collateralToken, address _indexToken, bool _isLong) private view {
        if (_isLong) {
            require(_collateralToken == _indexToken, "42");
            require(whitelistedTokens[_collateralToken], "43");
            require(!stableTokens[_collateralToken], "44");
            return;
        }

        require(whitelistedTokens[_collateralToken], "45");
        require(stableTokens[_collateralToken], "46");
        require(!stableTokens[_indexToken], "47");
        require(shortableTokens[_indexToken], "48");
    }
    ...
    function _increasePoolAmount(address _token, uint256 _amount) private {
        ...
        require(poolAmounts[_token] <= balance, "49");
    ...
    function _validateBufferAmount(address _token) private view {
        if (poolAmounts[_token] < bufferAmounts[_token]) {
            revert("poolAmount < buffer");
        }
    }

    function _increaseUsdrAmount(address _token, uint256 _amount) private {
        usdrAmounts[_token] = usdrAmounts[_token].add(_amount);
        uint256 maxUsdrAmount = maxUsdrAmounts[_token];
        if (maxUsdrAmount != 0) {
            require(usdrAmounts[_token] <= maxUsdrAmount, "51");
        }
        emit IncreaseUsdrAmount(_token, _amount);
    }

    function _decreaseUsdrAmount(address _token, uint256 _amount) private {
        uint256 value = usdrAmounts[_token];
        // since USDR can be minted using multiple assets
        // it is possible for the USDR debt for a single asset to be less than zero
        // the USDR debt is capped to zero for this case
        if (value <= _amount) {
            usdrAmounts[_token] = 0;
            emit DecreaseUsdrAmount(_token, value);
            return;
        }
        usdrAmounts[_token] = value.sub(_amount);
        emit DecreaseUsdrAmount(_token, _amount);
    }

    function _increaseReservedAmount(address _token, uint256 _amount) private {
        reservedAmounts[_token] = reservedAmounts[_token].add(_amount);
        require(reservedAmounts[_token] <= poolAmounts[_token], "52");
        emit IncreaseReservedAmount(_token, _amount);
    }

    function _decreaseReservedAmount(address _token, uint256 _amount) private {
        reservedAmounts[_token] = reservedAmounts[_token].sub(_amount, "insufficient reserve");
    ...
    function _onlyGov() private view {
        require(msg.sender == gov, "53");
    }

    // we have this validation as a function instead of a modifier to reduce contract size
    function _validateManager() private view {
        if (inManagerMode) {
            require(isManager[msg.sender], "54");
        }
    }
    ...
    function _validateGasPrice() private view {
        ...
        require(tx.gasprice <= maxGasPrice, "55");
    }

    function _onlyGovOrWrapper() private view {
        if (wrapper != address(0)) {
            require(msg.sender == gov || msg.sender == wrapper, "56");
        } else {
            require(msg.sender == gov, "57");
        }
    }
}