...
contract OrderBook is ReentrancyGuard, IOrderBook {
    ...
    uint256 public constant USDR_PRECISION = 1e18;
    ...
    function initialize(
        ...
        emit Initialize(_router, _vault, _weth, _usdr, _minExecutionFee, _minPurchaseTokenAmountUsd);
    }
    ...
    function getUsdrMinPrice(address _otherToken) public view returns (uint256) {
        // USDR_PRECISION is the same as 1 USDR
        uint256 redemptionAmount = IVault(vault).getRedemptionAmount(_otherToken, USDR_PRECISION);
    ...
    function validateSwapOrderPriceWithTriggerAboveThreshold(
        ...
        // 1. USDR doesn't have a price feed so we need to calculate it based on redepmtion amount of a specific token
        // That's why USDR price in USD can vary depending on the redepmtion token
        // 2. In complex scenarios with path=[USDR, BNB, BTC] we need to know how much BNB we'll get for provided USDR
        // to know how much BTC will be received
        // That's why in such scenario BNB should be used to determine price of USDR
        if (tokenA == usdr) {
            // with both _path.length == 2 or 3 we need usdr price against _path[1]
            tokenAPrice = getUsdrMinPrice(_path[1]);
        } else {
    ...
    function _vaultSwap(address _tokenIn, address _tokenOut, uint256 _minOut, address _receiver) private returns (uint256) {
        ...
        if (_tokenOut == usdr) { // buyUSDR
            amountOut = IVault(vault).buyUSDR(_tokenIn, _receiver);
        } else if (_tokenIn == usdr) { // sellUSDR
            amountOut = IVault(vault).sellUSDR(_tokenOut, _receiver);
        } else { // swap
