...
contract FastPriceFeed is ISecondaryPriceFeed, IFastPriceFeed, Governable {
    ...
    modifier onlySigner() {
        require(isSigner[msg.sender], "FastPriceFeed: not signer");
        _;
    }

    modifier onlyUpdater() {
        require(isUpdater[msg.sender], "FastPriceFeed: not updater");
        _;
    }
    ...
    function setPrices(address[] memory _tokens, uint256[] memory _prices, uint256 _timestamp) external override onlyUpdater {
    ...
    function getPrice(address _token, uint256 _refPrice, bool _maximise) external override view returns (uint256) {
        // secondary price feed is out of date, use refPrice with spread
        ...
        // secondary price feed is not updated with price duration, use refPrice with spread
        ...
        // fast price is not enabled, use refPrice
