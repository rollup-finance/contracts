...
contract Reader is Governable {
    ...
    function getMaxAmountIn(IVault _vault, address _tokenIn, address _tokenOut) public view returns (uint256) {
        ...
        uint256 maxUsdrAmount = _vault.maxUsdrAmounts(_tokenIn);

        if (maxUsdrAmount != 0) {
            if (maxUsdrAmount < _vault.usdrAmounts(_tokenIn)) {
                return 0;
            }

            uint256 maxAmountIn = maxUsdrAmount.sub(_vault.usdrAmounts(_tokenIn));
            maxAmountIn = maxAmountIn.mul(10 ** tokenInDecimals).div(10 ** USDR_DECIMALS);
    ...
    function getAmountOut(IVault _vault, address _tokenIn, address _tokenOut, uint256 _amountIn) public view returns (uint256, uint256) {
        ...
        {
            uint256 usdrAmount = _amountIn.mul(priceIn).div(PRICE_PRECISION);
            usdrAmount = usdrAmount.mul(10 ** USDR_DECIMALS).div(10 ** tokenInDecimals);
            ...
            uint256 feesBasisPoints0 = _vault.getFeeBasisPoints(_tokenIn, usdrAmount, baseBps, taxBps, true);
            uint256 feesBasisPoints1 = _vault.getFeeBasisPoints(_tokenOut, usdrAmount, baseBps, taxBps, false);
    ...
    function getVaultTokenInfo(address _vault, address _weth, uint256 _usdrAmount, address[] memory _tokens) public view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
            ...
            amounts[i * propsLength + 3] = vault.getRedemptionAmount(token, _usdrAmount);
    ...
    function getFullVaultTokenInfo(address _vault, address _weth, uint256 _usdrAmount, address[] memory _tokens) public view returns (uint256[] memory) {
    ...
    function getPositions(address _vault, address _account, address[] memory _collateralTokens, address[] memory _indexTokens, bool[] memory _isLong) public view returns(uint256[] memory) {
        ...
        for (uint256 i = 0; i < _collateralTokens.length; i++) {
            ...
            uint256 size2 = amounts[i * POSITION_PROPS_LENGTH];
            uint256 averagePrice2 = amounts[i * POSITION_PROPS_LENGTH + 2];
            uint256 lastIncreasedTime2 = amounts[i * POSITION_PROPS_LENGTH + 6];
            if (averagePrice2 > 0) {
                (bool hasProfit, uint256 delta) = IVault(_vault).getDelta(_indexTokens[i], size2, averagePrice2, _isLong[i], lastIncreasedTime2);
